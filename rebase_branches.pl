...
say "\nNot koha-plugin-edifact-hein, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-hein";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-hein';
    next if $repo eq 'koha-plugin-edifact-hein-docs';
